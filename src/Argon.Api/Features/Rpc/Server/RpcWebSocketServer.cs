...
public class RpcWebSocketServer(
    ...
    TokenAuthorization tokenAuthorization) : RpcServiceBase(services)
{
    ...
    public RpcServerConnectionFactory       ServerConnectionFactory { get; } = services.GetRequiredService<RpcServerConnectionFactory>();
    ...
    public async Task Invoke(HttpContext context, bool isBackend)
    {
        ...
        try
        {
            ...
            var properties              = PropertyBag.Empty.Set((RpcPeer)peer).Set(context).Set(webSocket);
            ...
            var channel = new WebSocketChannel<RpcMessage>(webSocketChannelOptions, webSocketOwner, cancellationToken)
            {
                OwnsWebSocketOwner = false
            };
            connection = await ServerConnectionFactory.Invoke(peer, channel, properties, cancellationToken).ConfigureAwait(false);

            if (peer.IsConnected())
            {
                var delay = Settings.ChangeConnectionDelay;
                Log.LogWarning("{Peer} is already connected, will change its connection in {Delay}...", peer, delay.ToShortString());
    ...
    public record Options
    {
        ...
        public Func<WebSocketAcceptContext> ConfigureWebSocket { get; init; } = () => new WebSocketAcceptContext();
    }
}