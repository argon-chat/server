...
/// missing key, two kinds on one layer, two claiming the same legacy field — stops the process at
...
public sealed class CosmeticsFeature : IArgonFeature
{
    ...
    public void Configure(ArgonFeatureContext ctx)
    {
        ...
        // Singleton because it holds the kill-switch cache; everything it touches per call comes from
        // a fresh DbContext.
        ctx.Services.AddSingleton<ICosmeticProfileProjection, CosmeticProfileProjection>();

        // Stateless: the context it writes through is either opened per call or handed in by whoever
        // owns the transaction.
        ctx.Services.AddSingleton<CosmeticGrantService>();
    }
}
