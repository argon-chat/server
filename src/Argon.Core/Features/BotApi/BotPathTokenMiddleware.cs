...
/// <c>/{token}/IMessages/v1/Send</c> → extracts token into
/// <c>Authorization: Bot {token}</c> header and rewrites path to
/// <c>/IMessages/v1/Send</c> so routing works normally.
/// </summary>
public sealed class BotPathTokenMiddleware(RequestDelegate next)
{
    // Match bot tokens (HEX:BASE64URL) after / — interface names start with "I" so no ambiguity
