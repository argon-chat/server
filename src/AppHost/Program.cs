...
var api = builder.AddProject<Argon_Api>("argonapi").WithReference(apiDb, "DefaultConnection").WithReference(cache).WithReference(clickhouse)
   ...
   .WithEnvironment("Jwt__Key", jwtKey).WithEnvironment("Jwt__Expire", "228").WithExternalHttpEndpoints();
